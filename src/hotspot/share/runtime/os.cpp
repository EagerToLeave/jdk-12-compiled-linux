...
static bool conc_path_file_and_check(char *buffer, char *printbuffer, size_t printbuflen,
    ...
    const char *filesep = (WINDOWS_ONLY(lastchar == ':' || )
    lastchar == os::file_separator()[0]) ?
    "" : os::file_separator();
...
void os::init_before_ergo() {
    ...
    // Yellow Zone 的默认大小为 8k, 可以通过 -XX:StackYellowPages 来指定
    // Red Zone 的默认大小为 4k, 可以通过 -XX:StackRedPages 来指定
    // 这 12k 的权限都是 PROT_NONE, 也就是不可读不可写不可执行
    // 读写这一块区域都会触发 Segmentation Fault（SIGSEGV）
    // JVM 为了能自己处理栈溢出异常，它处理了 SIGSEGV 这个信号
    // Yellow zone：这一块区域是用来处理可恢复的栈溢出的，当栈溢出发生在这一块区域时，
    //              会把这 8k 的内存区域的权限改为可读可写，随后 JVM 会抛出 StackOverflowError 异常，
    //              StackOverflowError 这个异常应用层可以被捕获进行处理。
    //              当异常抛出处理完以后，这 8k 内存区域的权限又会恢复为不可读、不可写、不可执行的状态。
    // Red zone：这一块的区域是用来处理不可恢复的栈溢出的，算是线程栈最后的防线了
    //          这个区域的栈溢出，JVM 会视为致命错误，进程会退出并生成 hs_err_pid.log 文件
    //          当栈溢出在这个区域时，会首先把这 4k 的权限改为可读可写，以便留一些栈空间生成 hs_err_pid.log 文件
...
void *os::malloc(size_t size, MEMFLAGS memflags, const NativeCallStack &stack) {
    ...
    if ((intptr_t) ptr == (intptr_t) MallocCatchPtr) {
        log_warning(malloc, free)("os::malloc caught, "
        SIZE_FORMAT
        " bytes --> "
        PTR_FORMAT, size, p2i(ptr));
        breakpoint();
    }
    debug_only(
...
void os::print_summary_info(outputStream *st, char *buf, size_t buflen) {
    ...
    if (mem == 0) {  // for low memory systems
        ...
        st->print("%d cores, "
        SIZE_FORMAT
        "M, ", processor_count(), mem);
    } else {
        st->print("%d cores, "
        SIZE_FORMAT
        "G, ", processor_count(), mem);
    }
...
void os::print_location(outputStream *st, intptr_t x, bool verbose) {
    ...
    if (Universe::heap()->is_in(addr)) {
        ...
        if (o != NULL) {
            if ((HeapWord *) o == (HeapWord *) addr) {
                ...
                st->print(INTPTR_FORMAT
                " is pointing into object: ", p2i(addr));
            }
            o->print_on(st);
            return;
        }
        ...
        " is an unallocated location in the heap", p2i(addr));
        return;
    }
    ...
    if (align_down((intptr_t) addr, sizeof(intptr_t)) != 0 && accessible) {
        if (JNIHandles::is_global_handle((jobject) addr)) {
            st->print_cr(INTPTR_FORMAT
            " is a global jni handle", p2i(addr));
            return;
        }
        if (JNIHandles::is_weak_global_handle((jobject) addr)) {
            st->print_cr(INTPTR_FORMAT
            " is a weak global jni handle", p2i(addr));
            return;
        }
#ifndef PRODUCT
        ...
        if (JNIHandles::is_local_handle((jobject) addr)) {
            st->print_cr(INTPTR_FORMAT
            " is a local jni handle", p2i(addr));
            return;
        }
#endif
    }
    ...
    for (JavaThreadIteratorWithHandle jtiwh; JavaThread * thread = jtiwh.next();) {
        ...
        if (thread->on_local_stack(addr)) {
            st->print_cr(INTPTR_FORMAT
            " is pointing into the stack for thread: "
    ...
    if (Metaspace::contains(addr)) {
        if (Klass::is_valid((Klass *) addr)) {
            st->print_cr(INTPTR_FORMAT
            " is a pointer to class: ", p2i(addr));
            ...
            st->print_cr(INTPTR_FORMAT
            " is pointing into metadata", p2i(addr));
        }
        return;
    }
    ...
    if (accessible) {
        st->print(INTPTR_FORMAT
        " points into unknown readable memory:", p2i(addr));
...
bool os::set_boot_path(char fileSep, char pathSep) {
    ...
    char *jimage = format_boot_path("%/lib/"
    MODULES_IMAGE_NAME, home, home_len, fileSep, pathSep);
