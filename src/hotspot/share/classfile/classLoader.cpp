...
ClassPathImageEntry::ClassPathImageEntry(JImageFile *jimage, const char *name) :
    ...
    _name = NEW_C_HEAP_ARRAY(
    const char, len, mtClass);
    ...
    if (location == 0) {
        ...
        if (pkg_name != NULL) {
            if (!Universe::is_module_initialized()) {
                ...
                PackageEntry *package_entry = ClassLoader::get_package_entry(name,
                ...
                if (package_entry != NULL) {
                    ...
                    if (module_name != NULL) {
                        location = (*JImageFindResource)(_jimage, module_name, get_jimage_version_string(), name,
                                                         &size);
                    }
                }
            }
...
void ClassLoader::load_jimage_library() {
    ...
    JImageResourceIterator = CAST_TO_FN_PTR(JImageResourceIterator_t,
                                            os::dll_lookup(handle, "JIMAGE_ResourceIterator"));
...
// 通过BootstrapClassLoader 加载class文件
/**
 *  ApplicationClassLoader --> PlatformCLassLoader --> BootstrapClassloader  -->
 *                           ApplicationClassLoader <-- PlatformCLassLoader <--
 *  双亲委派机制:
 *  虚拟机加载类时首先使用 ApplicationClassLoader进行加载, ApplicationClassLoader会将任务委派给
 *  PlatformCLassLoader进行加载, PlatformCLassLoader又会将任务委派给BootstrapClassloader执行
 *  BootstrapClassloader如果找到指定的类, 则直接返回, 如果未找到则会依次返回给PlatformCLassLoader、
 *  ApplicationClassLoader, 每个类加载器对应固定的搜索路径, 如果上述过程执行完毕仍然未找到对应类, 则
 *  抛出CLassNotFundException;
 *  双亲委派机制核心作用:
 *     避免类被重复加载, 保证java核心类, 如java.lang.Object, java.lang.Thread等只能通过BootstrapClassLoader
 *     进行加载
 *  jvm正常情况下类加载过程如下:
 *      在JVM里有一个数据结构叫做SystemDictionary, 这个结构主要就是用来检索我们常说的类信息
 *      这些类信息对应的结构是klass, 对SystemDictionary的理解，可以认为就是一个Hashtable
 *      key是类加载器对象+类的名字, value是指向klass的地址
 *      这样当我们任意一个类加载器去正常加载类的时候,就会到这个SystemDictionary中去查找
 *      看是否有这么一个klass可以返回,如果有就返回它,否则就会去创建一个新的并放到结构里,其中委托类加载过程如上所述
 *  特殊情况:
 *      unsafe.defineAnonymousClass(Class<?> hostClass, byte[] data, Object[] cpPatches)
 *      defineAnonymousClass这个方法比较特别,从名字上也看得出，是创建了一个匿名的类，不过这种匿名的概念和我们理解的匿名是不太一样的
 *      这种类的创建通常会有一个宿主类, 也就是第一个参数指定的类, 这样一来，这个创建的类会使用这个宿主类的定义类加载器来加载这个类
 *      最关键的一点是这个类被创建之后并不会丢到上述的SystemDictionary里,也就是说我们通过正常的类查找
 *      比如Class.forName等api是无法去查到这个类是否被定义过的; 因此过度使用这种api来创建这种类在一定程度上会带来一定的内存泄露
 *      根据JSR292，jvm通过InvokeDynamic可以支持动态类型语言, 这样一来其实我们可以提供一个类模板
 *      在运行的时候加载一个类的时候先动态替换掉常量池中的某些内容, 这样一来，同一个类文件，我们通过加载多次
 *      并且传入不同的一些cpPatches，也就是defineAnonymousClass的第三个参数, 这样就能做到运行时产生不同的效果
 *      主要是因为原来的JVM类加载机制是不允许这种情况发生的, 因为我们对同一个名字的类只能被同一个类加载器加载一次
 *      为了能支持动态语言的特性, Unsafe类型才提供类似的api以实现此类功能
 */
...
void ClassLoader::create_javabase() {
...

    {
        ...
        ModuleEntry *jb_module = null_cld_modules->locked_create_entry_or_null(Handle(),
                                                                               false, vmSymbols::java_base(), NULL,
                                                                               NULL, null_cld);
        if (jb_module == NULL) {
