...
Klass *
SystemDictionary::resolve_or_fail(Symbol *class_name, Handle class_loader, Handle protection_domain, bool throw_error,
                                  TRAPS) {
...
InstanceKlass *SystemDictionary::resolve_super_or_fail(Symbol *child_name,
    ...
    bool throw_circularity_error = false;
    {
        ...
        if (!throw_circularity_error) {
            ...
            PlaceholderEntry *newprobe = placeholders()->find_and_add(p_index, p_hash, child_name, loader_data,
                                                                      PlaceholderTable::LOAD_SUPER, super_name, THREAD);
        }
    }
    if (throw_circularity_error) {
        ResourceMark rm(THREAD);
...
void SystemDictionary::double_lock_wait(Handle lockObject, TRAPS) {
    ...
    assert((!oopDesc::equals(lockObject(), _system_loader_lock_obj) && !is_parallelCapable(lockObject)),
           "unexpected double_lock_wait");
    ...
    if (!class_has_been_loaded) {
...

        {
            ...
            if (!throw_circularity_error && !class_has_been_loaded) {
                PlaceholderEntry *newprobe = placeholders()->find_and_add(p_index, p_hash, name, loader_data,
                                                                          PlaceholderTable::LOAD_INSTANCE, NULL,
                                                                          THREAD);
        ...
        if (load_instance_added == true) {
            ...
            MutexLocker mu(SystemDictionary_lock, THREAD);
            placeholders()->find_and_remove(p_index, p_hash, name, loader_data, PlaceholderTable::LOAD_INSTANCE,
                                            THREAD);
            SystemDictionary_lock->notify_all();
        }
    }
...
InstanceKlass *SystemDictionary::find_or_define_instance_class(Symbol *class_name, Handle class_loader,
...

    {
        ...
        probe = placeholders()->find_and_add(p_index, p_hash, name_h, loader_data, PlaceholderTable::DEFINE_CLASS, NULL,
                                             THREAD);
        ...
        if (is_parallelDefine(class_loader) && (probe->instance_klass() != NULL)) {
            placeholders()->find_and_remove(p_index, p_hash, name_h, loader_data, PlaceholderTable::DEFINE_CLASS,
                                            THREAD);
            SystemDictionary_lock->notify_all();
#ifdef ASSERT
            InstanceKlass* check = find_class(d_hash, name_h, dictionary);
...
bool SystemDictionary::do_unloading(GCTimer *gc_timer) {
    ...
    {
        GCTraceTime(Debug, gc, phases)
        t("ResolvedMethodTable", gc_timer);
...
void SystemDictionary::resolve_well_known_classes(TRAPS) {
    ...
    WKID
    last = NOT_JVMCI(WKID_LIMIT)
    JVMCI_ONLY(FIRST_JVMCI_WKID);
...
static bool is_always_visible_class(oop mirror) {
    ...
    return klass->is_public() &&
           (InstanceKlass::cast(klass)->is_same_class_package(SystemDictionary::Object_klass()) ||       // java.lang
            InstanceKlass::cast(klass)->is_same_class_package(
                    SystemDictionary::MethodHandle_klass()));  // java.lang.invoke
}
